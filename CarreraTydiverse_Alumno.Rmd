...
```{r}
# Transformar los datos en un tibble
cran <- as_tibble(mydf)

# Borrar el data frame original
rm(mydf)
...
# Usar la función glimpse para obtener una vista detallada de los datos
glimpse(cran)
...
# Seleccionar solo las variables ip_id, package y country
...
cran3 <- mutate(cran3, size_gb = size / 2^30)
...
top_counts <- cran %>%
  ...
  filter(downloads > quantile(downloads, probs = 0.99)) %>%
  pull(package)

# Muestra los paquetes top descargas
top_counts
...
# Muestra los paquetes top descargas ordenados
top_counts_sorted
...
# Muestra los paquetes top descargas únicas ordenados
top_unique_sorted
